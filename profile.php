    ...
    // DEBUG: Log what's in session before fetching
    error_log("Profile page - Before getUserById - Session status: " . ($_SESSION['status'] ?? 'NOT SET'));

    // Get user data from database (this will also update session)
    $currentUser = getUserById($currentUserId, true);

    // DEBUG: Log what we got from database
    error_log("Profile page - After getUserById - currentUser status: " . ($currentUser['status'] ?? 'NOT SET'));
    error_log("Profile page - After getUserById - Session status: " . ($_SESSION['status'] ?? 'NOT SET'));
    ...
    // If status is missing from both database result and session, fetch it directly
    if (!isset($currentUser['status']) || empty($currentUser['status'])) {
        if (!isset($_SESSION['status']) || empty($_SESSION['status'])) {
            // Fetch status directly from database as a last resort
            try {
                $pdo = getDatabaseConnection();
                ...
                if ($statusResult) {
                    $currentUser['status'] = $statusResult['status'];
                    $_SESSION['status'] = $statusResult['status'];
                    error_log("Profile page - Fetched status from database: " . $statusResult['status']);
                } else {
                    $currentUser['status'] = 'active';
                    error_log("Profile page - Could not fetch status, defaulting to active");
                }
            } catch (Exception $e) {
                error_log("Profile page - Error fetching status: " . $e->getMessage());
                $currentUser['status'] = 'active';
            }
        } else {
            $currentUser['status'] = $_SESSION['status'];
            error_log("Profile page - Using session status: " . $_SESSION['status']);
        }
    }
                        ...
                        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex items-center space-x-4 mb-6">
                                    ...
                                    <div>
                                        <h2 class="text-xl font-bold text-gray-900 dark:text-white"><?php echo htmlspecialchars($currentUser['username']); ?></h2>
                                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300"><?php echo htmlspecialchars($currentUser['email']); ?></p>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold mt-1
                                            <?php
                                            $userRole = $currentUser['role'] ?? 'unknown';
                                            switch ($userRole) {
                                                case 'admin':
                                                    echo 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200';
                                                    break;
                                                case 'farmer':
                                                    echo 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-200';
                                                    break;
                                                case 'student':
                                                    echo 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200';
                                                    break;
                                                default:
